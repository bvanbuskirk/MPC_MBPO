...
class FFModel(nn.Module, BaseModel):
    ...
    def forward(
        ...
        next_obs_unnormalized = obs_unnormalized + delta_pred_unnormalized
        return (next_obs_unnormalized, delta_pred_normalized)

    def get_prediction(self, obs, acs, data_statistics, ret_numpy=True):
        """
        ...
        pred, _ = self(obs, acs, obs_mean, obs_std, acs_mean, acs_std, delta_mean, delta_std)
        if ret_numpy:
          pred = ptu.to_numpy(pred)
        return pred
    ...
    def update(self, observations, actions, next_observations, data_statistics):
        ...
        next_obs_unnorm, delta_pred_norm = self(obs, acs, 
                                                obs_mean, 
                                                obs_std,
                                                acs_mean,
                                                acs_std,
        ...
        loss = self.loss(delta_pred_norm, ptu.from_numpy(target))
